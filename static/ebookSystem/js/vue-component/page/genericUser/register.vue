...
<template>
	<div id="genericUser_register">
		...
		<div class="form-group">
			...
			<div class="col-sm-7">
				<input type="button" class="btn btn-primary" value="註冊" @click="checkInput()">
				<input type="reset" class="btn btn-danger" value="清除" @click="clearInput()">
			</div>
		</div>
	</div>
</template>
	...
	module.exports = {
		...
		methods: {
			checkInput: function(){
				...
				//firstname
				let firstname = this.infos.first_name;
				if(firstname===''){
					err.push('姓氏不得為空');
				}

				//lastname
				let lastname = this.infos.last_name;
				if(lastname===''){
					err.push('名字不得為空');
				}

				//email
				let email = this.infos.email;
				if(email===''){
					err.push('電子信箱不得為空');
				}
				else{
