...
# ['book_shelf']
BoolStatusFilter = KeyMapAttrFilterFactory(key='status',
	type=convert_bool,
	attr='status')

# ['book_person', book_shelf ,]
OwnerFilter = KeyMapAttrFilterFactory(key='owner_id',
	type=str,
	attr='owner_id')

# ['school',]
OrgFilter = KeyMapAttrFilterFactory(key='org_id', type=str, attr='org_id')
CategoryFilter = KeyMapAttrFilterFactory(key='category_id',
	type=str,
	attr='category_id')

#
EbookOrgFilter = KeyMapAttrFilterFactory(key='org_id',
	type=str,
	attr='book__org_id')

# ['service', 'serviceinfo_record', ]
EditorFilter = KeyMapAttrFilterFactory(key='editor_id',
	type=str,
	attr='editor_id')

# ['book_person', ]
BookInfoOwnerFilter = KeyMapAttrFilterFactory(key='owner_id',
	type=str,
	attr='book__owner_id')

BookInfoOrgFilter = KeyMapAttrFilterFactory(key='org_id',
	type=str,
	attr='book__org_id')
BookInfoCategoryFilter = KeyMapAttrFilterFactory(key='category_id',
	type=str,
	attr='book__category_id')

#['book_manager']
BooknameFilter = KeyMapAttrFilterFactory(key='bookname',
	type=convert_unicode,
	attr='book_info__bookname__contains')
...
class ICFilter(filters.BaseFilterBackend):
	def filter_queryset(self, request, queryset, view):
		...
		try:
			index_category_id = int(
				request.query_params.get('index_category_id'))
			if index_category_id == 0:
				return queryset.filter(book__index_category=None)
			else:
				index_category = IndexCategory.objects.get(
					id=index_category_id)
				return queryset.filter(
					book__index_category_id__in=index_category.descendants_id)
...
class CBCFilter(filters.BaseFilterBackend):
	def filter_queryset(self, request, queryset, view):
		try:
			chinese_book_category = int(
				request.query_params.get('chinese_book_category'))
		except:
			chinese_book_category = -1
		if chinese_book_category >= 0 and chinese_book_category < 10:
			CBC = chinese_book_category * 100
			return queryset.filter(chinese_book_category__gt=CBC - 1,
				chinese_book_category__lt=CBC + 100)
		else:
			return queryset
...
class NewestFilter(filters.BaseFilterBackend):
	def filter_queryset(self, request, queryset, view):
		...
		if newest:
			return queryset.order_by('-book__finish_date',
				'-date')[0:int(newest)]
		else:
			return queryset
...
class HottestFilter(filters.BaseFilterBackend):
	def filter_queryset(self, request, queryset, view):
		...
		if hottest:
			...
			r = GetBookRecord.objects.filter(get_time__gt=begin_day,
				get_time__lt=end_day).values('book').annotate(
				count=Count('book'))
			import heapq
			hottest_ISBN = heapq.nlargest(int(hottest),
				r,
				key=lambda s: s['count'])
			book_list = [
				Book.objects.get(ISBN=i['book']) for i in hottest_ISBN
			]
