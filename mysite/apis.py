...
class Statistics(APIView):
	...
	def filter_time(self, request):
		...
		if begin_time:
			...
			self.begin_time = datetime.datetime(temp_time[0],
				temp_time[1],
				temp_time[2],
				tzinfo=utc)
		...
		if end_time:
			...
			temp_time = [int(i) for i in temp_time]
			self.end_time = datetime.datetime(temp_time[0],
				temp_time[1],
				temp_time[2],
				tzinfo=utc)
	...
	def summary(request, template_name='mysite/statistics.html'):
		...
		for month in month_list:
			...
			editor_count_30 = User.objects.filter(
				is_editor=True,
				date_joined__lte=month,
				online__gt=month - datetime.timedelta(days=30),
				auth_email=True,
				auth_phone=True,
			).count()
			finish_count = Book.objects.filter(finish_date__lte=month,
				upload_date__lte=month,
				source='self').count()
			txt_count = Book.objects.filter(upload_date__lte=month,
				source='txt').count()
			epub_count = Book.objects.filter(upload_date__lte=month,
				source='epub').count()
			book_count = Book.objects.filter(upload_date__lte=month).count()
			scanbook_count = Book.objects.filter(upload_date__lte=month,
				source='self').count()
			result.append({
				'month': month,
	...
	def user_download(self, request):
		...
		r = [{
			'groupfield':
			User.objects.get(id=i['user']).first_name +
			User.objects.get(id=i['user']).last_name,
			'count':
			i['count'],
	...
	def book_download_pd(self, request):
		...
		groupby = query.annotate(month=TruncMonth('get_time')).values(
			'month', 'book').annotate(count=Count('id'))
		df = pd.DataFrame(groupby)
		if df.empty:
			res['result'] = list()
			return Response(data=res, status=status.HTTP_202_ACCEPTED)
		df['month'] = df['month'].apply(lambda x: str(x)[0:10])
		df_month = df.pivot(index='book', columns='month',
		...
		df = pd.merge(df, df_book, how='left', left_on='book',
			right_on='ISBN').fillna('').drop('ISBN', axis=1)
	...
	def user_download_pd(self, request):
		...
		groupby = query.annotate(month=TruncMonth('get_time')).values(
			'month', 'user').annotate(count=Count('id'))
		df = pd.DataFrame(groupby)
		if df.empty:
			res['result'] = list()
			return Response(data=res, status=status.HTTP_202_ACCEPTED)
		df['month'] = df['month'].apply(lambda x: str(x)[0:10])
		df_month = df.pivot(index='user', columns='month',
			values='count').reset_index()
		...
		df = pd.merge(df, df_user, how='left', left_on='user',
			right_on='id').fillna('').drop('id', axis=1)
		df = df.rename(columns={
			'username': 'index',
			'count': 'all',
			'user': 'id'
	...
	def user_editrecord_pd(self, request):
		...
		if hasattr(self, 'org'):
			query = query.filter(editor__org=self.org)
		groupby = query.annotate(month=TruncMonth('get_date')).values(
			'month', 'editor').annotate(count=Count('id'))
		df = pd.DataFrame(groupby)
		if df.empty:
			res['result'] = list()
			return Response(data=res, status=status.HTTP_202_ACCEPTED)
		df['month'] = df['month'].apply(lambda x: str(x))
		df_month = df.pivot(index='editor', columns='month',
			values='count').reset_index()
		...
		s = pd.Series({'id': -1, 'username': '全部'})
		df_user = df_user.append(s, ignore_index=True)
		df = pd.merge(df, df_user, how='left', left_on='editor',
			right_on='id').fillna('').drop('id', axis=1)
		df = df.rename(columns={
			'username': 'index',
			'count': 'all',
	...
	def get(self, request, action=None, *args, **kwargs):
		...
		if action:
			if action == 'resource' and 'resource' in kwargs:
				return self.resource(request, kwargs['dir'],
					kwargs['resource'])
