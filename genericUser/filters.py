...
DisabilityCardActiveFilter = KeyMapAttrFilterFactory(key='is_active',
	type=convert_bool,
	attr='is_active')

# ['', ]
ServiceInfoExchangeFilter = KeyMapAttrFilterFactory(key='is_exchange',
	type=convert_bool,
	attr='is_exchange')
...
DisabilityCardOrgFilter = KeyMapAttrFilterFactory(key='org_id',
	type=str,
	attr='owner__org_id')

QAndACategoryFilter = KeyMapAttrFilterFactory(key='category',
	type=str,
	attr='category')
	...
	def filter_queryset(self, request, queryset, view):
		...
		if auth:
			if auth == 'false' or auth == 'False':
				return queryset.filter(is_guest=True).filter(
					disabilitycard_set=None)
			elif auth == 'true' or auth == 'True':
				return queryset.filter(is_guest=True).exclude(
					disabilitycard_set=None)
			else:
				return queryset
		else:
